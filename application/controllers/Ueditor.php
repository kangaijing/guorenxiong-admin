...
class UeditorController extends AdminbaseController {
	...
	private function uploadimage() {
		$config = array(
				...
				'rootPath' 	=> './uploads/',
				'savePath' 	=> 'article/',	 // 设置附件上传目录
				'saveName' 	=> array('date', round(microtime(true) * 1000)),
		...
		if($upres) {
			// 获取图片路径
			$url = '/uploads/' . $upres['upfile']['savepath'] . $upres['upfile']['savename'];
			$title = $oriName = $upres['upfile']['savename'];
	...
	private function get_remote_image() {
		...
		foreach($source as $imgUrl) {
			...
			$fileShow = '/uploads/article/' . $date . '/' . $file;  // 返回的图片地址，不要 '.'
			$file = './uploads/article/' . $date . '/' . $file;
